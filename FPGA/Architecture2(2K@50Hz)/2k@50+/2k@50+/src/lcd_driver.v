...
//		frame_count <= 1'b0;
	...
	else if(fifo_en)//data in fifo are enough
		begin
        ...
        lcd_hs <= (1)? ((hcnt <= H_SYNC - 1'b1) ? 1'b0 : 1'b1) : 1'b0;
		end
end 
...
//		frame_count <= 1'b0;
	...
	else if(hcnt == H_TOTAL - 1'b1 && fifo_en)		//line over   //fifo are enough
		begin
...
//			frame_count <= ~frame_count;
        ...
        lcd_vs <= (1)? ((vcnt <= V_SYNC - 1'b1) ? 1'b0 : 1'b1) : 1'b0;
		end
end
...
always@(posedge clk) begin
	lcd_en		<=	(1)?   ( (hcnt >= H_SYNC + H_BACK  && hcnt < H_SYNC + H_BACK + H_DISP) &&
...
//	Standard FIFO Request Port
 always@(posedge clk) begin
	lcd_request	<=	(1)?  ((hcnt >= H_SYNC + H_BACK - H_AHEAD + (H_DISP-H_REAL)/2 && hcnt < H_SYNC + H_BACK + (H_DISP+H_REAL)/2 - H_AHEAD) &&
						 	(vcnt >= V_SYNC + V_BACK + (V_DISP-V_REAL)/2 && vcnt < V_SYNC + V_BACK + (V_DISP+V_REAL)/2) 
						 	? 1'b1 : 1'b0) : 1'b0;
